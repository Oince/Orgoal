...
public class SignUpController {
    ...
    @PostMapping
    ResponseEntity<Message> signUp(@RequestBody SignUpForm signUpForm) {
        ...
        try {
            ...
            Message error = new Message(HttpStatus.CONFLICT.value(), e.getMessage(), new Date());
            return new ResponseEntity<>(error, HttpStatus.CONFLICT);
        }
        catch (NoSuchAlgorithmException e) {
            //해시 실패시 예외 발생, 500 코드 리턴
            Message error = new Message(HttpStatus.INTERNAL_SERVER_ERROR.value(), e.getMessage(), new Date());
