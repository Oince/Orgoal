<template>
  ...
  <div v-if="showApplyForm" class="activity-apply-form-container">
    <div class="activity-apply-form">
      ...
      <p class="buttons">
        <button @click.prevent="doApply()" class="button blue">신청</button>
...
export default {
  ...
  setup: function () {
    ...
    let showApplyForm = ref(false);
    let applyComment = ref("");

    // methods
    let onClickApplyButton = function () {
      showApplyForm.value = true;
      console.log("참가신청 버튼");
    };
    let doApply = function () {
      const URI = `/api/activity/${activityID}`;
      let applyInfo = {
        answer: applyComment.value,
      };
      let config = {
        headers: {
